...
function renderBlock(input: Parameters<SFCVueRenderFunction>[0], block: EndgeTooltipMarkdownBlock, key: number) {
  if (block.kind === 'heading') return input.h(`h${block.level}`, { key, class: 'endge-tooltip__heading' }, inline(input, block.children))
  if (block.kind === 'paragraph') return input.h('p', { key, class: 'endge-tooltip__paragraph' }, inline(input, block.children))
  if (block.kind === 'code-block') return input.h('pre', { key, class: 'endge-tooltip__code-block' }, [input.h('code', null, block.value)])
...
function inline(input: Parameters<SFCVueRenderFunction>[0], nodes: EndgeTooltipMarkdownInline[]): any[] {
  return nodes.map((node, index) => {
    ...
    if (node.kind === 'strong') return input.h('strong', { key: index }, inline(input, node.children))
    if (node.kind === 'emphasis') return input.h('em', { key: index }, inline(input, node.children))
